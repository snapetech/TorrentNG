...
Environment:
  ...
  TNG_STORAGE_SKIP_URING         set to 1 to skip io_uring graduation
  TNG_STORAGE_SKIP_MOVE_IMPORT   set to 1 to skip real-root move/import
...
run_gate "storage hardware matrix" "$ROOT/scripts/storage_hardware_matrix.sh" "$@"

if [[ "${TNG_STORAGE_SKIP_URING:-0}" == "1" ]]; then
  mark "io_uring graduation" SKIP "TNG_STORAGE_SKIP_URING=1"
else
  run_gate "io_uring graduation" "$ROOT/scripts/storage_uring_graduation.sh" "$uring_target"
fi
...
run_gate "storage certification index" "$ROOT/scripts/storage_certification_index.sh"

{
  ...
  echo "- Keep `TNG_STORAGE_SKIP_URING=0` and `TNG_STORAGE_SKIP_MOVE_IMPORT=0` for release evidence."
  echo
