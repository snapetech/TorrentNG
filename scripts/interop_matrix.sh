...
run_tracker_outage_after_peer_discovery_case() {
  local status="PASS" fixture torrent info_hash
  ...
  add_to_client torrentngd "$torrent" || status="FAIL"
  bridge_client_peer_to_rust transmission "$info_hash" || status="FAIL"
  sleep "${INTEROP_TRACKER_OUTAGE_AFTER_SECS:-3}"
  compose stop -t 10 opentracker >/dev/null || status="FAIL"
  wait_explicit_peer_complete "$TIMEOUT_LOCAL" "$fixture" "$info_hash" transmission transmission torrentngd || status="FAIL"
  verify_fixture_hashes torrentngd "$fixture" || status="FAIL"
  append_report "- Seeder: transmission"
  append_report "- Leecher: torrentngd"
  append_report "- Fixture: single-64m"
  append_report "- Tracker stopped after: ${INTEROP_TRACKER_OUTAGE_AFTER_SECS:-3}s"
  append_report "- Peer source after outage: explicit known peer"
  append_report "- Info hash: $info_hash"
  append_report "- Status: **$status**"
  append_report ""
  [[ "$status" == "PASS" ]]
...
run_qbit_mutation_facade_case() {
  ...
  curl --max-time "$CURL_MAX_TIME" -fsS -H "Authorization: Bearer $RUST_TOKEN" "$(client_url torrentngd)/api/qb/v2/torrents/webseeds?hash=$info_hash" |
    jq -e 'type == "array"' >/dev/null || status="FAIL"
  curl --max-time "$CURL_MAX_TIME" -fsS -H "Authorization: Bearer $RUST_TOKEN" "$(client_url torrentngd)/api/qb/v2/torrents/pieceStates?hash=$info_hash" |
    jq -e 'type == "array"' >/dev/null || status="FAIL"
