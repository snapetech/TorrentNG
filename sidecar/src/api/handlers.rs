...
const MAX_RTORRENT_CUSTOM_RC_BYTES: usize = MAX_RTORRENT_OVERLAY_BYTES - 16 * 1024;
...
pub async fn set_rtorrent_settings(
    ...
    if patch
        .custom_rc
        .as_deref()
        .is_some_and(|custom_rc| custom_rc.len() > MAX_RTORRENT_CUSTOM_RC_BYTES)
    {
        errors.push(format!(
            "custom_rc exceeds the {MAX_RTORRENT_CUSTOM_RC_BYTES}-byte limit"
        ));
    }
...
pub async fn set_user_agent(
    ...
    let ua = match crate::config::normalize_runtime_user_agent(&body.user_agent) {
        Ok(ua) => ua,
...
mod tests {
    ...
    #[test]
    fn rtorrent_overlay_write_rejects_oversized_output_before_replacement() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("rtorrent.rc");
        let error = write_rtorrent_overlay(
            &path,
            &rtorrent_settings(),
            &BTreeMap::new(),
            &"x".repeat(MAX_RTORRENT_OVERLAY_BYTES),
        )
        .expect_err("oversized overlay should be rejected");
