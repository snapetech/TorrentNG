...
impl Client {
    pub async fn list_files(&self, hash: &str) -> Result<Vec<RawFile>> {
        let result = self.call("f.multicall", &[
            hash.into(),
            "".into(),
            "f.path=".into(),
            "f.size_bytes=".into(),
            ...
            "f.is_created=".into(),
            "f.is_open=".into(),
        ]).await.with_context(|| format!("f.multicall {hash}"))?;
    ...
    pub async fn set_file_priority(&self, hash: &str, file_index: usize, priority: i64) -> Result<()> {
        ...
        self.call("f.priority.set",
            &[hash.into(), XmlValue::Int(file_index as i64), XmlValue::Int(priority)]
        ).await?;
        Ok(())
    }
}
