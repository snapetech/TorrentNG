...
- The current renamed mainline scheduler has peer-read readahead and the
  standalone `DeviceElevator` policy, but cross-request elevator dispatch is
  not wired into `MountScheduler`.
- The HDD run proves current mainline already removes fd churn and gets an
  8x backend-read reduction for adjacent peer reads through readahead.
- The shuffled baseline intentionally disables readahead and records the
  current random-read behavior that the future wired elevator must beat.
