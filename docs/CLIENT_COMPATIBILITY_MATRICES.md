...
| App info | `app/version`, `webapiVersion`, `buildInfo`, `preferences`, `setPreferences`, `shutdown`, `sendTestEmail`, `getCookies`, `setCookies`, `rotateAPIKey`, `deleteAPIKey`, `networkInterfaceList`, `networkInterfaceAddressList`, `defaultSavePath` | Same | TorrentNG-client/compatible-client mix; listen_port/dht/pex preferences are applied by the TorrentNG client, other broad preference keys are projection-only, shutdown is wired to daemon signal handling, and test email is explicit `501` unsupported | Probe every endpoint plus preference/cookie/API-key persistence |
