...
| P1 | `native_rest_mutation_matrix` | Native REST mutation coverage for start/stop aliases, metadata update, delete-with-data semantics, category, tags, file priority/file rename, tracker add/edit/remove, per-torrent limits, global transfer limits, and files/trackers/limits projection is covered by `rt-api-native` tests plus the `rust-qbit-mutation-facade` Docker row in `scripts/interop_matrix.sh` |
...
| Done | Backfill native REST mutation endpoints used by overlay adapters and compatibility facades: start/stop aliases, torrent metadata update, delete data flag, file priority/file rename, tracker patching, category, tags, per-torrent limits, and global transfer limits | Native REST mutation matrix |
