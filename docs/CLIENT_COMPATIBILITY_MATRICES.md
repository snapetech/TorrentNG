...
| P1 | `storage_resume_matrix` | stop mid-transfer/restart/resume and corrupt block recheck repair are covered by Docker protocol rows `resume-after-partial-download` and `force-recheck-corruption-repair`; missing file recovery remains planned; local storage topology coverage is included in `scripts/universal_compatibility_certification.sh` |
