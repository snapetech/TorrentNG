...
- The Windows storage-plan executor opens configured roots as directory
  capabilities and performs traversal, recursive copy/verification/deletion,
  pruning, reconciliation, and no-replace rename relative to opened directory
  handles. Reparse points are rejected during handle-relative traversal.
...
| Windows storage-plan path authority | The Windows executor is handle-relative under configured root capabilities for path traversal, recursive copy/verification/deletion, pruning, reconciliation, and no-replace rename. It rejects reparse points while opening each path component. `rt-storage` and its test target cross-check for `x86_64-pc-windows-msvc`. | Native Windows runtime and race behavior have not been exercised in this environment. | Implementation complete. Native Windows runtime qualification is evidence-only follow-up. |
