...
| Backend queue bounding | The scheduler has bounded queue semaphores, but `PreadBackend` and `UringBackend` use unbounded `std::sync::mpsc::channel` internally. | Once live scheduler traffic uses `DiskBackend`, backend queues need their own fail-closed bound for callers outside the scheduler and for future integration mistakes. | Replace backend channels with bounded queues or require explicit queue permits before backend submission. |
| Native storage config coverage | Native TOML exposes only a subset of the scheduler/runtime knobs; most `StorageIoConfig` fields remain defaulted unless tests construct schedulers manually. | Operators cannot tune file-pool size, I/O/hash workers, queue depth, durability mode, preallocation mode, or readahead from config on production hosts. | Extend `[storage]` config with the remaining `StorageIoConfig` fields and pass them into torrent tasks and v2 recheck schedulers. |
