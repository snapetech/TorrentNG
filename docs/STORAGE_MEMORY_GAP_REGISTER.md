...
| Scheduler/backend double queue | Scheduler direct reads, writes, and syncs now await backend completion outside the scheduler blocking pool after a short open/metadata phase. Prepare, sparse extent probing, and peer-read elevator dispatch still use the older bounded blocking bridge. | The hottest payload and durability paths are improved, but secondary paths can still occupy scheduler workers while waiting on backend completion. | Apply the same guard/open/backend split to elevator dispatch where it matters, then remove blocking backend waits from scheduler code. |
