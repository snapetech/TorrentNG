...
| --- | --- | --- | --- |
| Zero-copy storage read adoption | Scheduler reads and peer-read elevator reads borrow from the process-level frame pool, upload returned blocks and scheduler-owned cache entries are memory-leased, storage exposes `scheduled_read_owned`, and upload block assembly consumes frame-owned exact backend reads. Compatibility callers can still request plain `Bytes`. | The remaining risk is unnecessary copy cost in lower-priority callers that have not opted into frame-owned reads. | Migrate additional measured hot call sites from `scheduled_read` to `scheduled_read_owned` where profiling shows the compatibility copy matters. |
| Move/import product workflow breadth | The engine save-path path, native storage-plan API, and first WebUI planner now share the durable checkpointed storage-plan job path. Remaining breadth is bulk operation ergonomics and richer progress display for large library batches. | The basic workflow is exposed, but large maintenance operations may still need templates, batch selection, and progress affordances. | Extend from real operator feedback rather than inventing batch controls ahead of usage. |
