...
- [ ] Convert `EngineHandle::recheck_torrent` from direct command into a durable job.
- [ ] Store resumable recheck checkpoint: file index, piece index, byte offset, verified bytes, invalid pieces.
...
- [ ] Emit `check_started`, `check_progress`, `check_completed`, and failure events.
...
- [ ] Persist tracker state across restart.
- [ ] Persist uploaded/downloaded/left accounting per torrent.
- [ ] Send `started`, `completed`, and `stopped` events exactly once where required.
- [ ] Add restart jitter to prevent announce storms.
- [ ] Add scrape support/state where tracker supports it.
- [ ] Classify tracker failures and warnings into durable state.
...
- [ ] Add native health endpoint backed by engine readiness.
...
- [ ] Add structured "why is this not seeding?" diagnostic API.
