...
| P2 | Move/import product workflow breadth | A first guided UI workflow now previews and executes move/import/delete plans from the Library storage panel. Remaining breadth is bulk selection, operation templates, and richer progress surfaces for large library batches. | Large operator workflows may still need more guardrails than a single-plan form. | Extend the UI only after real operator runs identify the batch controls and progress fields that matter. |
