...
| --- | --- | --- | --- | --- |
| P2 | Zero-copy returned-read adoption | Storage now exposes a frame-owned read API while keeping `scheduled_read -> Bytes` compatibility. Upload block assembly and sparse recheck extent reads use the owned path; remaining `scheduled_read` users are compatibility wrappers and tests. | Avoids forcing a broad API churn before measurements identify paths where frame ownership materially improves throughput or memory. | Profile release workloads and migrate any new measured hot call sites from `scheduled_read` to `scheduled_read_owned` as evidence warrants. |
