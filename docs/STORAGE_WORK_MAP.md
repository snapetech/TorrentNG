...
| --- | --- | --- | --- | --- |
| P2 | Zero-copy returned-read adoption | Storage now exposes a frame-owned read API while keeping `scheduled_read -> Bytes` compatibility. Upload block assembly and sparse recheck extent reads use the owned path; remaining `scheduled_read` users are compatibility wrappers and tests, and `scripts/storage_ng_feature_matrix.sh` fails if new production callers appear. | Avoids forcing a broad API churn before measurements identify paths where frame ownership materially improves throughput or memory. | Profile release workloads and migrate any new measured hot call sites from `scheduled_read` to `scheduled_read_owned` as evidence warrants. |
| P1 | `io_uring` fixed-buffer/frame integration | `UringBackend` reports its fixed-buffer strategy explicitly: current registered fixed buffers are `worker_copy`, not `frame_pool_slots`. | Operators can no longer confuse fixed-buffer kernel support with application-frame zero-copy, but the extra copy remains until frame-pool slot leases exist. | Add frame-pool slot leases with stable registered buffer indexes, then benchmark `pread` vs `uring` on target hardware before changing `auto`. |
| P2 | Move/import product workflow breadth | The Library storage panel previews and executes move/import/delete plans, includes affected-torrent metadata, accepts completed-step indexes for resumed durable jobs, and summarizes forward/rollback step counts and bytes. Remaining breadth is operation templates and richer live progress surfaces for large library batches. | Large operator workflows may still need more guardrails than a single-plan form. | Extend from real operator runs once batch templates and progress fields are proven useful. |
