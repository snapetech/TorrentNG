...
| --- | --- | --- | --- | --- |
| P2 | Zero-copy returned-read adoption | Storage now exposes a frame-owned read API while keeping `scheduled_read -> Bytes` compatibility, and upload block assembly uses the owned path. Remaining callers need selective migration only where profiling shows the compatibility copy matters. | Avoids forcing a broad API churn before measurements identify paths where frame ownership materially improves throughput or memory. | Migrate additional measured hot call sites from `scheduled_read` to `scheduled_read_owned` as evidence warrants. |
