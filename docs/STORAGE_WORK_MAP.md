...
| P0 | Bounded backend queues | `PreadBackend`/`UringBackend` use `std::sync::mpsc::channel`, which is unbounded. The live scheduler has a queue semaphore, but the backend itself has no independent bounded send path. | Once `MountScheduler` uses `DiskBackend`, backend queue growth must remain bounded even for non-scheduler runtime callers. | Replace backend channels with bounded queues or require an explicit permit object before backend submission; expose queue-full errors instead of relying on caller-side throttling only. |
