...
| Save-path move execution | Implemented for engine field updates | `begin_update_torrent_fields`/`finish_prepared_torrent_fields` detach filesystem planning; `move_torrent_payload_files` executes the durable plan; qBit/native set-location paths now move existing payload files before committing `save_path` |
