...
| P0 | Finish scheduler/backend double-queue removal | Scheduler direct reads, writes, and syncs now hold scheduler permits while awaiting backend completion directly after a short blocking open/metadata phase. Prepare, sparse extent probing, and peer-read elevator dispatch still use the scheduler blocking bridge. | The hottest payload and durability paths are no longer tying up a scheduler worker while backend I/O runs, but secondary paths still have the older bridge shape. | Apply the same guard/open/backend split to peer-read elevator dispatch where it matters, then retire blocking backend waits from scheduler code. |
