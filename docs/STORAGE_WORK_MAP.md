...
| P2 | Move/import API breadth | Engine save-path updates now execute payload moves through durable storage-plan jobs; broader import/delete API surfaces are still explicit helper/certification paths. | UI/API workflows that expose library import or delete planning should reuse the same durable storage-plan job path instead of ad hoc filesystem mutations. | Add native API endpoints for move/import/delete plan preview and execution when product workflow requires them. |
