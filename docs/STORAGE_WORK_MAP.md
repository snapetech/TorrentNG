...
| P2 | Move/import product workflow breadth | Engine save-path updates and native storage-plan preview/execute APIs now use durable storage-plan jobs; richer UI workflows are still future product surface. | Operators may still need a guided UI around the lower-level native endpoints for large library operations. | Add UI workflow affordances when operator demand justifies it. |
