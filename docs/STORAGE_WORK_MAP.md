...
| --- | --- | --- | --- |
| Torrent payload I/O | `rt-storage::MountScheduler` | Download writes, seed reads, recheck reads, v2 file verification, fastresume syncs | This is the production torrent hot path. Each scheduler owns per-class semaphores, dirty generations, peer-read cache/counters, and the HDD peer-read elevator; path-backed schedulers share the bounded file pool, positioned-I/O backend/workers, and hash workers for one process/configuration. |
...
| --- | --- | --- |
| Positioned reads/writes | Implemented through a shared `SelectedDiskBackend` resource used by path-backed schedulers | `crates/rt-storage/src/scheduler.rs` `read_at`, `write_at`; `crates/rt-storage/src/backend.rs`; `concurrent_positioned_writes_do_not_share_cursor` |
...
| Bounded blocking workers | Implemented through shared path-backed `BlockingPool`s plus per-scheduler admission semaphores | `crates/rt-storage/src/scheduler.rs`; `full_mount_queue_fails_closed`, `queued_disk_bytes_track_active_blocking_job_payload`, `path_schedulers_share_bounded_worker_resources` |
| Open-file pool | Implemented as a shared path-backed cache for the scheduler hot path | `FilePoolStats`; `file_pool_records_hits_and_evictions`; `path_schedulers_share_bounded_worker_resources`; real-device `repeated_reads_reuse_one_open_file_handle` |
...
| Hash isolation | Implemented with a shared bounded hash pool for path-backed schedulers and independent per-scheduler admission | `hash_sha1`, `hash_v2_leaf`, `hash_v2_root`; `path_schedulers_share_bounded_worker_resources`; scale tests for hash-pool isolation |
...
| Restartable move/import primitive | Implemented at the storage-plan executor and engine job boundaries | `execute_storage_plan_with_checkpoints`; `execute_storage_plan_under_roots_with_checkpoints`; copy-based move/import writes to staged paths before final rename; checkpointed steps are validated against live filesystem state before resume and only then skipped; engine storage-plan jobs persist queued/running/checkpoint/`commit_pending`/completed state, with save-path moves held in `commit_pending` until the actor-side durable row update |
| Save-path move execution | Implemented for engine field updates | `begin_update_torrent_fields`/`finish_prepared_torrent_fields` detach filesystem planning; `move_torrent_payload_files` executes the durable plan; qBit/TorrentNG set-location paths now move existing payload files before committing `save_path` |
| Payload deletion | Implemented through the durable storage worker | `remove_torrent_inner` queues a root-confined `SafeDeleteIfPresent` plan; `StorageDeleteFinished` finalizes metadata/registry cleanup and recovery is idempotent |
| TorrentNG storage plan API | Implemented for preview and execution | `/api/v1/storage/plan`; `/api/v1/storage/execute`; execution goes through durable TorrentNG-client storage-plan jobs |
