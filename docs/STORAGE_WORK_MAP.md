...
| --- | --- | --- | --- | --- |
| P2 | Zero-copy returned-read ownership | Scheduler reads and elevator reads now borrow from the process-level frame pool, upload returned blocks and scheduler-owned cache entries are leased, but generic `scheduled_read` still returns plain `Bytes`. | The hot upload/cache lifetimes are accounted; full zero-copy ownership would require a public storage read payload type instead of `bytes::Bytes`. | Add a frame-owned bytes API only if measurements show the remaining copy matters. |
