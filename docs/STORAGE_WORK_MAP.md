...
| Restartable move/import primitive | Implemented at the storage-plan executor boundary | `execute_storage_plan_with_checkpoints`; `execute_storage_plan_under_roots_with_checkpoints`; checkpointed steps are skipped on resume and newly completed steps are reported to the caller for durable persistence |
...
| --- | --- | --- | --- | --- |
| P0 | One storage memory accounting path | Scheduler reads and elevator reads now borrow from the process-level frame pool, but returned `Bytes` are still copied out of frames. | Storage frame caps now bound backend read buffers in the primary torrent path, but the returned payload lifetime is not represented as a frame lease. | Add a frame-owned bytes API or equivalent lease-carrying payload type if zero-copy storage-frame ownership becomes required. |
| P0 | Finish scheduler/backend double-queue removal | Scheduler direct reads, writes, syncs, and peer-read elevator dispatch now hold scheduler permits while awaiting backend completion directly after a short blocking open/metadata phase. Prepare and sparse extent probing still use the scheduler blocking bridge. | The hottest payload, durability, and elevator paths no longer tie up a scheduler worker while backend I/O runs, but secondary metadata/preparation paths still have the older bridge shape. | Apply the same guard/open/backend split to remaining paths that perform backend I/O, then retire blocking backend waits from scheduler code. |
...
| P1 | Cross-device move/import resume in engine DB | The storage executor can now resume from completed step indexes and report new checkpoints, but the engine has not yet wired those checkpoints into its SQLite job rows. | Multi-TB moves need persisted progress accounting across process restarts, not just an in-memory callback. | Add a storage-move job kind that writes plan steps and completed step indexes through `rt-db::JobRow` before calling the checkpointed executor. |
