...
| P2 | Move/import product workflow breadth | The Library storage panel previews and executes move/import/delete plans, includes affected-torrent metadata, accepts completed-step indexes for resumed durable jobs, offers move/import/delete/resume templates, and summarizes forward/rollback step counts and bytes. Remaining breadth is richer live progress surfaces for large library batches. | Large operator workflows may still need more guardrails than a single-plan form. | Extend from real operator runs once progress fields are proven useful. |
