...
| P0-4 Type inference | **Fixed** | Root cause: the SQL `LIKE` pattern for "tv" included a literal `"s%e%"` glob meant to approximate SxxExx markers — in SQL `LIKE`, that matches any string containing an 's' anywhere before an 'e' anywhere later, i.e. most English text. Replaced the whole `LIKE`-glob classifier with a word-boundary-aware Rust classifier (`sidecar/src/media_type.rs`) registered as a SQLite scalar function, with a real SxxExx digit-run detector. Unit-tested against the exact false positives found live (the Grisham ebook, the FLAC album). Requires a sidecar rebuild to take effect. |
