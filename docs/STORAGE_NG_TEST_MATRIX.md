...
| Storage core | fd pool, positioned I/O, short-I/O error mapping, preallocation, checkpoint and strict durability metrics, page-cache advice, sparse recheck, readahead, topology, elevator policy | `cargo test -p rt-storage` |
...
| TorrentNG API metrics | Prometheus projection for storage backend, frame/fd runtime, scheduler counters, bounded peer-cache pressure, peer buffer bytes, resource-governor classes | `cargo test -p rt-api-native render_metrics_includes_engine_stats` |
| Move/import executor | plan admission, symlink-safe no-overwrite moves, copy-based move source cleanup, staged copy import, hardlink-or-copy import, recursive directory copy/delete, rename/copy/import symlink rejection, symlink-safe delete, staged rollback cleanup, storage-root confinement, optional real-root fixture execution | `scripts/storage_move_import_certification.sh`; set `TNG_STORAGE_MOVE_IMPORT_ROOT` for hardware-root evidence |
...
| Release evidence wrapper | Hardware matrix, `io_uring` graduation, real-root move/import, and certification index are run as one operator command | `scripts/storage_release_certification.sh /target/root [...]` |
...
| `torrentng_peer_{rx,tx}_buffer_bytes`, `torrentng_peer_command_queue_*`, `torrentng_memory_class_used_bytes{class="peer_buffer"}` | Outstanding peer request/send buffers, upload block and queued-bitfield leases, and bounded peer command queues are visible for memory-pressure accounting |
...
- `scripts/storage_certification_selftest.sh` passes and proves FAIL reports are
  not indexed as passing storage evidence.
- `scripts/storage_release_certification.sh /target/root [...]` passes on
  release hardware before publishing storage performance claims.
