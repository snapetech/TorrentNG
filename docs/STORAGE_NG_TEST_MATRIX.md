...
| Move/import executor | plan admission, no-overwrite moves, copy-based move source cleanup, hardlink-or-copy import, recursive directory copy/delete, staged rollback cleanup, storage-root confinement, optional real-root fixture execution | `scripts/storage_move_import_certification.sh`; set `TNG_STORAGE_MOVE_IMPORT_ROOT` for hardware-root evidence |
